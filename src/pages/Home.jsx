...
export default function Home() {
  ...
  const DEFAULT_AVATAR =
    'https://firebasestorage.googleapis.com/v0/b/social-app-8a28d.appspot.com/o/default-avatar.png?alt=media&token=78165d2b-f095-496c-9de2-5e143bfc41cc';
  ...
  const handleEditReply = async (postId, commentIndex, replyIndex, newText) => {
    const post = posts.find((p) => p.id === postId);
    const updatedComments = [...post.comments];
  ...
  const handleReply = async (postId, commentIndex) => {
    ...
    const replyText = replyContents[replyKey];
    ...
    setShowReplyForms((prev) => ({ ...prev, [replyKey]: false }));
  };
                      ...
                      <div className="flex-1">
                        ...
                        <div className="mt-1 flex space-x-2 text-xs">
                          ...
                          <button
                            ...
                            className="text-blue-600 hover:underline"
                          >
                            {showReplyForms[replyKey] ? 'Cancel' : 'Reply'}
                          </button>
                          {hasReplies && (
                            <button
                              onClick={() => toggleShowReplies(post.id, i)}
                            ...
                            >
                              {showReplyForms[replyKey] ? 'Hide replies' : `View replies (${comment.replies.length})`}
                            </button>
                          )}
                        </div>
                        ...
                        {/* Replies */}
                        {(showReplyForms[replyKey] || hasReplies) && comment.replies && (
                          <div className="ml-4 mt-2 space-y-2">
                            {comment.replies.map((reply, ri) => {
                              ...
                              const isEditing = editCommentMap[replyEditKey] !== undefined;
                                ...
                                <div key={ri} className="flex items-start space-x-2">
                                  ...
                                  <div className="flex-1">
                                    ...
                                    {isEditing ? (
                                      <div>
                                        <textarea
                                          value={editCommentMap[replyEditKey]}
                                          onChange={(e) =>
                                            setEditCommentMap((prev) => ({
                                              ...prev,
                                          ...
                                          <button
                                            onClick={() => {
                                              handleEditReply(
                                                ...
                                                i, 
                                                ri, 
                                                editCommentMap[replyEditKey]
                                              );
                                              setEditCommentMap((prev) => ({ 
                                                ...prev, 
                                                [replyEditKey]: undefined 
                                              }));
                                            }}
                                            className="text-xs bg-blue-500 text-white px-2 py-0.5 rounded"
                                          >
