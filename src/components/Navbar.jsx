...
export default function Navbar() {
  ...
  return (
    <nav className={`shadow p-4 mb-6`} style={{ backgroundColor: theme.navbarColor }}>
      <div className="flex justify-between items-center max-w-6xl mx-auto">
        ...
        {!shouldHideContent && user && (
          <div className={`hidden sm:flex items-center gap-4 ${textColorClass}`}>
            ...
            {/* Requests dropdown (right side next to notifications) */}
            <FriendRequestsMenu />
      ...
      {menuOpen && !shouldHideContent && user && (
        <div className={`sm:hidden mt-4 flex flex-col gap-2 ${textColorClass}`}>
          ...
          {/* Requests dropdown inside mobile menu */}
          <div className="pt-2">
            <FriendRequestsMenu />
          </div>
