...
import { collection, onSnapshot, query, where, getDocs, orderBy, startAt, endAt, limit } from 'firebase/firestore';
...
debounceRef.current = setTimeout(async () => {
  setLoading(true);
  try {
    const variants = Array.from(new Set([t, capitalize(t), t.toLowerCase(), t.toUpperCase()]));
    const userPromises = variants.map((v) =>
      getDocs(query(collection(db, 'users'), orderBy('displayName'), startAt(v), endAt(v + '\uf8ff'), limit(20)))
    );
    const groupPromises = variants.map((v) =>
      getDocs(query(collection(db, 'groups'), orderBy('name'), startAt(v), endAt(v + '\uf8ff'), limit(20)))
    );
    ...
    const userCandidates = Array.from(
      new Map(
        userSnaps
          .flatMap((s) => s.docs)
          .map((d) => [d.id, { id: d.id, displayName: d.data().displayName || d.data().username || 'Unknown', photoURL: d.data().photoURL }])
      ).values()
    );
    const groupCandidates = Array.from(
      new Map(
        groupSnaps
          .flatMap((s) => s.docs)
          .map((d) => [d.id, { id: d.id, name: d.data().name || 'Group', logoURL: d.data().logoURL }])
      ).values()
    );
    ...
    if (users.length === 0) {
      const snap = await getDocs(query(collection(db, 'users'), orderBy('displayName'), limit(50)));
      const more = snap.docs
        .map((d) => ({ id: d.id, displayName: d.data().displayName || d.data().username || 'Unknown', photoURL: d.data().photoURL }))
        .filter((u) => (u.displayName || '').toLowerCase().startsWith(tLower))
        .slice(0, 5);
      if (more.length > 0) users.push(...more);
    }
    if (groups.length === 0) {
      const snap = await getDocs(query(collection(db, 'groups'), orderBy('name'), limit(50)));
      const more = snap.docs
        .map((d) => ({ id: d.id, name: d.data().name || 'Group', logoURL: d.data().logoURL }))
        .filter((g) => (g.name || '').toLowerCase().startsWith(tLower))
        .slice(0, 5);
...
return (
<nav className="shadow p-4 mb-6" style={{ backgroundColor: theme?.navbarColor || '#ffffff' }}>
...
{!shouldHideContent && user && (
      <button onClick={() => setMenuOpen(!menuOpen)} className={`sm:hidden focus:outline-none ${textColorClass}`}>
      ...
      <div className={`hidden sm:flex items-center gap-4 ${textColorClass}`}>
        ...
        <div className="relative">
          <input
            ...
            onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); goFirstResult(); } }}
            type="text"
            placeholder="users, groups"
            className="w-32 bg-white text-gray-900 placeholder-gray-500 rounded pl-8 pr-3 py-1 text-sm shadow outline-none focus:ring-2 focus:ring-blue-500"
          />
          <SearchIcon size={14} className="absolute left-2 top-1/2 -translate-y-1/2 text-gray-500 pointer-events-none" />
          {focus && term.trim().length >= 2 && (
            <div className="absolute z-50 mt-1 w-[10rem] bg-white text-gray-900 rounded shadow-lg border border-gray-200">
              ...
              {!loading && results.users.length > 0 && (
                <div className="py-1">
                  <div className="px-3 py-1 text-xs uppercase tracking-wider text-gray-400">Users</div>
                  {results.users.map((u) => (
                    <button
                      key={u.id}
                      onMouseDown={(e) => e.preventDefault()}
                      onClick={() => { navigate('/profile/' + u.id); setTerm(''); setFocus(false); }}
                      className="w-full text-left px-3 py-2 hover:bg-gray-100 flex items-center gap-2"
                    >
                      <img src={u.photoURL || '/images/avatar-placeholder.png'} alt="" className="w-6 h-6 rounded-full object-cover" />
              ...
              {!loading && results.groups.length > 0 && (
                <div className="py-1 border-t border-gray-100">
                  <div className="px-3 py-1 text-xs uppercase tracking-wider text-gray-400">Groups</div>
                  {results.groups.map((g) => (
                    <button
                      key={g.id}
                      onMouseDown={(e) => e.preventDefault()}
                      onClick={() => { navigate('/groups/' + g.id); setTerm(''); setFocus(false); }}
                      className="w-full text-left px-3 py-2 hover:bg-gray-100 flex items-center gap-2"
                    >
                      <img src={g.logoURL || '/images/group-placeholder.png'} alt="" className="w-6 h-6 rounded object-cover" />
  ...
  {menuOpen && !shouldHideContent && user && (
    <div className={`sm:hidden mt-4 flex flex-col gap-2 ${textColorClass}`}>
      <Link to="/profile" className="text-sm hover:underline flex items-center gap-1" onClick={() => setMenuOpen(false)}>
        <span>Profile</span>
        {incomingCount > 0 && (
          <span className="ml-1 inline-flex items-center justify-center rounded-full bg-rose-600 text-white text-[10px] leading-none px-1.5 py-0.5">
            {incomingCount}
      ...
      <div className="relative">
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === 'Enter') {
              e.preventDefault();
              navigate(results.users[0] ? '/profile/' + results.users[0].id : results.groups[0] ? '/groups/' + results.groups[0].id : '/');
              setMenuOpen(false);
            }
          }}
          type="text"
        ...
        />
        <SearchIcon size={16} className="absolute left-2 top-1/2 -translate-y-1/2 text-gray-500 pointer-events-none" />
        {focus && term.trim().length >= 2 && (
              ...
              <div className="py-1">
                ...
                {results.users.map((u) => (
                  <button
                    key={u.id}
                    onMouseDown={(e) => e.preventDefault()}
                    onClick={() => { navigate('/profile/' + u.id); setTerm(''); setFocus(false); setMenuOpen(false); }}
                    className="w-full text-left px-3 py-2 hover:bg-gray-100 flex items-center gap-2"
                  >
                    <img src={u.photoURL || '/images/avatar-placeholder.png'} alt="" className="w-6 h-6 rounded-full object-cover" />
                    <span className="text-sm">{u.displayName}</span>
              ...
              <div className="py-1 border-t border-gray-100">
                ...
                {results.groups.map((g) => (
                  <button
                    key={g.id}
                    onMouseDown={(e) => e.preventDefault()}
                    onClick={() => { navigate('/groups/' + g.id); setTerm(''); setFocus(false); setMenuOpen(false); }}
                    className="w-full text-left px-3 py-2 hover:bg-gray-100 flex items-center gap-2"
                  >
                    <img src={g.logoURL || '/images/group-placeholder.png'} alt="" className="w-6 h-6 rounded object-cover" />
                    <span className="text-sm">{g.name}</span>
